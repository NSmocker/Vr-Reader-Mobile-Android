...
public class ftRenderLightmap : EditorWindow//ScriptableWizard
{
    ...
    void OnGUI()
    {
        ...
        GUI.Label(aboutRect, new GUIContent("<color=#5073c9ff><b>Bakery - GPU Lightmapper by Mr F</b></color>", "Version 1.5. Go to author's site"), linkStyle);
        ...
        GUI.BeginGroup(new Rect(10, 10+y, 300, 120), "Settings", headerStyle);
        ...
        if (userRenderMode == RenderMode.Shadowmask)
        {
            ...
            isDistanceShadowmask = GUI.Toggle(new Rect(2, 40, 200, 20), isDistanceShadowmask, new GUIContent("Distance shadowmask", "Use complete real-time shadows close to camera."));
    ...
    IEnumerator RenderLightProbesFunc()
    {
        ...
        Texture2D tex = null;
        float[] pixels = null;
        LightingDataAsset newAsset = null;
#if UNITY_2017_3_OR_NEWER
#else
        ...
        if (userRenderMode == RenderMode.Shadowmask)
        {
            if (!useUnityForOcclsusionProbes) ftLightingDataGen.PatchShadowmaskLightingData(lmdataPath, newPath, ref tempID2RealID, ref realID2Channel);
        }
        else
        {
#endif
        ...
        {
            AssetDatabase.Refresh();
            newAsset = ApplyLightingDataAsset(newPath);
            EditorSceneManager.MarkAllScenesDirty();
    ...
    int SetupLightShadowmaskUsingBitmask(Light ulht, int bitmask, int[] channelBitsPerLayer)
    {
        ...
        // Setup the light
        SetupLightShadowmask(ulht, firstFreeBit);
	...
	IEnumerator RenderLightmapFunc()
	{
        ...
        */
...
#endif
                    startInfo2.FileName        = startInfo2.WorkingDirectory + "/ftrace.exe";
        ...
        if (userRenderMode == RenderMode.Shadowmask)
        {
            ...
            for(int i=0; i<All3.Length; i++)
            {
                ...
                SetupLightShadowmaskUsingBitmask(ulht, obj.bitmask, channelBitsPerLayer);
            }
            ...
            for(int i=0; i<lightsRemaining.Count; i++)
            {
                ...
                for(int j=0; j<lightsRemaining.Count; j++)
                {
                    ...
                    SetupLightShadowmask(lb, channelSet);
                }
        ...
        */
        ...
        foreach(var lmgroup in groupListPlain)
        {
            if (lmgroup.vertexBake && lmgroup.isImplicit) continue; // skip objects with scaleImLm == 0
            string nm;
            try
            {
                nm = lmgroup.name;
    ...
    IEnumerator RenderLMDirect(int LMID, string lmname, int resolution)
    {
        ...
        for(int i=0; i<All2.Length; i++)
        {
            ...
            */
            ...
            else
            {
                ...
                {
                    renderMode = obj.tangentSH ? "skytangentsh" : "skysh";
                }
                else
                {
                    renderMode = "sky";
        ...
        if (userRenderMode == RenderMode.Shadowmask && fcompIndirect.BaseStream.Position == 0)
        {
    ...
    */
    ...
    void SetupLightShadowmask(Light light, int channel)
    {
        if (channel > 3) Debug.LogWarning("Light " + light.name + " can't generate shadow mask (out of channels).");
